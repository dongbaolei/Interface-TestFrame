...
class Run_All():
    def sendmail(self, filenew):
        ...
        to_list = ['baolei.dong@lianzhongjinrong.cn']
        f = open(filenew, 'rb')
        ...
        f.close()
        # 定义标题
        msg['Subject'] = u"联众优车接口测试报告,请及时修改错误"
        # 定义发送时间（不定义的可能有的邮件客户端会不显示发送时间）
        msg['date'] = time.strftime('%a, %d %b %Y %H:%M:%S %z')
        msg['From'] = 'yibantester@163.com'
        smtp = smtplib.SMTP()
        # 连接SMTP服务器，此处用的126的SMTP服务器
        smtp.connect('smtp.163.com')
        # 发送邮件用户名和密码
        username = 'yibantester@163.com'
        password = 'bbs123456'
    ...
    def create_suite(self):
        ...
        listdir = 'E:\workspace\StockFinan-Frame\WebInterface-Frame\InterFace-TestCase'
...
if __name__ == '__main__':
    ...
    fp = open(filename, 'wb')
    #IOBase()
    runner = HTMLTestRunnerMH.HTMLTestRunner(stream=fp, title=u'联众优车接口测试报告', description=u'用例执行情况')
