...
# 组织-获取部门下级列表
class SysRoleGetDepartmentChildlist(unittest.TestCase):
    def setUp(self):
        pass

    def test_sysRoleGetDepartmentChildlist(self):
        randID = ''.join([random.choice(string.digits) for _ in range(1)])
        print(randID)
        url = "http://192.168.2.26:8080/sys/department/childlist/" + randID + ""
