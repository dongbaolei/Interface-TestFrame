...
sys.path.append('..\InterFace-TestCase\Contract_Images')
...
class Run_All():
    def sendmail(self, filenew):
        ...
        to_list = ['baolei.dong@lianzhongjinrong.cn','tech@lianzhongjinrong.cn']
        f = open(filenew, 'rb')
        ...
        f.close()
        # 定义标题
        msg['Subject'] = u"联众-合同影像平台接口测试报告,请及时修改错误"
    ...
    def create_suite(self):
        ...
        listdir = 'E:\workspace\StockFinan-Frame\WebInterface-Frame\InterFace-TestCase\Contract_Images'
...
if __name__ == '__main__':
    ...
    fp = open(filename, 'wb')
    #IOBase()
    runner = HTMLTestRunnerMH.HTMLTestRunner(stream=fp, title=u'联众-合同影像平台测试报告', description=u'用例执行情况')
