...
class Anxinq_Register(unittest.TestCase):
    def setUp(self):
        self.jira = JiraUtil.JiraUtil()
        self.data = test_Contract_Params.ContractParams()
        pass

    def test_A_right_Anxinq_Register(self):
        '''输入正确参数-产生合同'''
        ...
        rp = basehttp.post(httpconf.get_url(), eval(httpconf.get_headers()),  json.dumps(self.data.modify_anxinq_registerparams()), httpconf.get_files(),
